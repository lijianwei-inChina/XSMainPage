$(function() {
	if (/Android|webOS|iPhone|iPod|BlackBerry/i.test(navigator.userAgent)) {
		document.getElementById("gomocu").innerHTML = "<p class='normal-text'>您的设备不支持五子棋！</p>";
	} else {
		var game = {
			...
			winChesee: winner.getElementsByClassName("state-chess")[0],
			winName: winner.getElementsByClassName("chessName")[0],
			e: 0,
			chess_Board: [],
			chess_Name: ["黑棋", "白棋"],
			h: [],
			...
			drawLine: function() {
				...
				for (var i = 1; i <= 15; i++) {
					game.ctx.moveTo(i * 30 + .5, 450)
					game.ctx.lineTo(i * 30 + .5, 30)
					game.ctx.moveTo(30, i * 30 + .5)
					game.ctx.lineTo(450, i * 30 + .5)
			...
			canvasClick: function(e) {
				...
				if (dx < 1 || dx > 15 | dy < 1 || dy > 15) return;
				if (game.chess_Board[dx - 1][dy - 1] == 0) {
					...
					game.cName.innerText = game.e % 2 == 0 ? game.chess_Name[1] + "走" : game.chess_Name[
						0] + "走";
					game.sChesee.className = game.e % 2 == 1 ? "state-chess Bchess" :
						"state-chess Wchess";
					game.e++;
			...
			regret: function(e) {
				if (game.chessOff) {
					if (game.h.length > 0) {
						...
						let rmRm = obj.dm;
						rmRm.remove()
						game.cName.innerText = game.e % 2 == 0 ? game.chess_Name[1] + "走" : game
							.chess_Name[0] + "走";
						game.sChesee.className = game.e % 2 == 1 ? "state-chess Bchess" :
							"state-chess Wchess";
						game.e -= 1;
				...
				if (ms >= 5) {
					...
					continuity.forEach(function(value, index) {
						let time = setInterval(function() {
							game.lianz[value[0]][value[1]].style.transform =
								'scale(0.9)';
							game.lianz[value[0]][value[1]].style.boxShadow =
								"0px 0px 2px 2px #ffd507";
							s--;
					...
					game.h.forEach(function(value, index) {
						value.dm.innerText = value.list + 1;
					})

					li == 1 ? game.winChesee.className = "state-chess Wchess" : game.winChesee
						.className = "state-chess Bchess";
